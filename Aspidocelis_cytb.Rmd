...
la ruta del multifasta es: /Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612.fas
la carpeta dond evamos a trabajar es: /Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/
...
./remove_sample_number.sh /Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612.fas > /Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612_pop.fasta  
...
./remove_sample_tag.sh /Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612.fas > /Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612_all.fasta  
```
...
ARRAY=($(grep ">" /Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612.fas | cut -d "_" -f4 | tr -d ">" | sort | uniq))
for i in  ${ARRAY[*]}; 
do
grep "${i}" /Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612.fas -A 1  | grep -v "^--$" > /Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612_pop-${i}.fasta
done
```
...
setwd("/Users/Elena/ownCloud/Eurasian_lynx_phylogeography/mitogenomes_lynxes/haplotypes/")
...
#Per_pop samples
all.df <- list_haplotypes("/Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612.fas") %>%
  mutate (hap=paste("H_",sprintf("%02d",as.numeric(as.roman(as.character(hap)))),sep = "")    ,
          sample=pop )%>%
  separate(pop, c("sp", "locus", "samplecode", "pop"), sep = "_")  %>%
  arrange(hap) 
...
write.csv(ind.hap.pop, "/Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612_pop_haplotypes.csv", row.names = FALSE)

#Draw haplotypes piecharts per location
ind.hap.pop <- read.csv("/Users/jagodoy/Desktop/Reanalisis_Aspidoscelis_2018/Datasets/aspcytb317_612_pop_haplotypes.csv") %>%
   gather(pop, Freq, -hap)
$Si quieres contar por haplogrupo e vex de por haplotipo cambiar por qué agrupar
Hap <-   filter (ind.hap.pop, Freq>0) %>% group_by(., pop) %>% summarise(., sample.size = sum(Freq) ) #Counts how many haplotypes appear per population 
...
per_pop <- readData("/Users/jagodoy/ownCloud/Publico/Eurasian_lynx_phylogeography/mitogenomes_lynxes/diversity_neutrality_and_differentiation/fasta")
...
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_ya")%>%select(H)))
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_ki")%>%select(H)))
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_vl")%>%select(H)))
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_tu")%>%select(H)))
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_mo")%>%select(H)))
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_cr")%>%select(H)))
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_la")%>%select(H)))
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_po")%>%select(H)))
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_no")%>%select(H)))
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_ba")%>%select(H)))
count.Hap <- c(count.Hap, as.numeric(filter(Hap,pop == "c_ll_ur")%>%select(H)))
...
ind.hap <- ind.hap[c("c_ll_ya", "c_ll_ki", "c_ll_vl", "c_ll_tu", "c_ll_mo", "c_ll_cr", "c_ll_la", "c_ll_po", "c_ll_no", "c_ll_ba", "c_ll_ur")]
